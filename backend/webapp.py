    ...
    for event in calendar.walk("VEVENT"):
        ...
        eventsntimes.append(
            dict(
                ...
                start=cleanTime(str(event.get("DTSTART"))),
                end=cleanTime(str(event.get("dtend"))),
            )
        )
...
def getTemp():
    r = requests.get(
        url=f"http://api.openweathermap.org/data/2.5/forecast?id=524901&appid={apiKeys.getWeatherAPIkey()}"
    )
